...
const FindPage = () => {
    ...
    const {find, name, description, expedition, setExpedition, setDescription, setName, fetchFind, saveFind, sendFind, deleteFind, setFind, setFindId} = useFind()
        ...
        <div className="find-page-wrapper">
            ...
            <div className="find-places-wrapper">
                ...
                <div className="find-info-container">
                    ...
                    {[2, 3, 4].includes(find.status) && <span>Название: {find.name}</span> }
                    {[2, 3, 4].includes(find.status) && <span>Описание: {find.description}</span> }
                    {[2, 3, 4].includes(find.status) && <span>Экспедиция: {find.expedition}</span> }
                    <span>Дата создания: {formatDateTime(find.date_created)}</span>
                    {[2, 3, 4].includes(find.status) && <span>Дата формирования: {formatDateTime(find.date_formation)}</span>}
                    {completed && <span>Дата завершения: {formatDateTime(find.date_complete)}</span> }
                </div>
                ...
                <div className="inputs-container">
                    ...
                    <CustomInput placeholder="Экспедиция" value={expedition} setValue={setExpedition} disabled={!is_draft}  />
